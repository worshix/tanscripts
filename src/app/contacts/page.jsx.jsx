    ...
    return(
        <section className='contacts-page text-white mt-4'>
            ...
            <article className='the-contacts flex flex-col gap-2 sm:flex-row flex-wrap justify-center hero translate-x-full'>
                    {contacts.map((item) => {
                        return(
                            <Link to={item.link} id={item.id} className='flex flex-shrink-0 bg-white bg-opacity-10 w-[300px] mx-auto sm:mx-0 shadow-md sm:w-1/3'><i className={`bi bi-${item.bootstrapIcon} p-2  bg-white text-white rounded-r-md text-lg bg-opacity-15 block`}></i><div className='flex flex-col justify-center items-center w-4/5'><h2 className='font-bold'>{item.name}</h2>{item.details}<h2></h2></div></Link>
                        )
                    })}
            </article>
